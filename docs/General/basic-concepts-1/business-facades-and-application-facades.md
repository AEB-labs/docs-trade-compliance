...
> The term *Business Facade* is now replaced by *Business API*, and the term *Application Facade* is replaced by *UI API*. The old terms may still be used in some places for technical reasons, e.g. in class names of the SOAP API.