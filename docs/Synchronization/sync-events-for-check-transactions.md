...
* The deletion of a check transaction in the Export Controls web application, if the check transaction belongs to a subscribed partner system.
* The deletion or creation one of the items of a check transaction in the Export Controls web application, if the check transaction belongs to a subscribed partner system.
* The change of the check result from critical to uncritical (or to error) and vice versa for a transaction or one of the transaction items (e.g., after the creation of approvals) in the Export Controls web application, if the check transaction belongs to a subscribed partner system.
...
* businessObjectId:  A unique id for the transaction, the “transactionIdHost” that was provided by the partner system.
