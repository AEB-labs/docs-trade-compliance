...
{
  "type": "info",
  "body": "To make the synchronization API easier to use for Export Controls check transactions, some supplementary synchronization calls were added to the Export Controls API (see also Export Controls API usage scenario [Integration with Synchronization](doc:integration-with-synchronization))."
}
[/block]