...
metadata:
  robots: index
---
Lastly, in this less common use case, all steps of the screening process and the subsequent handling of any potential matches are fully integrated into a partner system. This requires multiple API calls to different API functions.
...
Once finalised, the user can trigger an API call to AEB Compliance Screening (either using the screenAddresses ([REST](ref:screenaddresses)) or the batchMatch ([SOAP](https://rz3.aeb.de/test4ce/servlet/bf/doc/RexBF/de/aeb/xnsg/rex/bf/IRexBF.html#batchMatch-de.aeb.xnsg.rex.bf.AddressPatternDTO:A-de.aeb.xnsg.rex.bf.MatchParametersDTO-)) API function). 
This will trigger the address screening algorithm and check all supplied addresses against restricted party lists with the results being logged automatically. The partner system will show whether a potential address match has been found or not.
