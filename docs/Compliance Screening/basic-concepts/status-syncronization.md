...
To do so, your partner system can send an additional API call to a specific [screening API function](ref:screenaddresses) for the addresses in question, which will respond with the statuses of your business partners after you have evaluated them.
...
A common use case looks like this: your partner system sends a scheduled API call to AEBs Compliance Screening (see [Bulk vs. Single Address Screening](doc:bulk-vs-single-address-screening)) and one of your business partners is found to possibly be on a restricted party list. You are required to evaluate that match and decide whether you are dealing with a false positive or not, for example by using AEBs Web Application. However, your decision needs to be fed back to your partner system, so false positively blocked business partners / transactions are released.

You can find a schematic representation of this [here](doc:screening-with-integrated-re-screen).
